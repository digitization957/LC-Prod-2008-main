...
(function () {
  ...
  function bodyDetail() {
    ...
    var fulfil = role === "owner" ? '<div class="panel panel-form"><h3>' + icon("check", 16) + " Fulfilment form</h3>" +
      '<div class="field" data-hl="flddate"><label>Completion date</label><div class="date-fy-row"><input type="date" value="2026-07-24" readonly /><span class="fy-chip">F26</span></div></div>' +
  ...
  function bodyCreateAgency() {
    return '<div class="modal" style="margin:24px auto 0" data-hl="agencyform"><div class="modal-head"><div class="icon-chip">' + icon("folder", 18) + '</div><div class="titles"><h3>Create agency</h3></div></div>' +
      '<div class="modal-body"><div class="field" data-hl="fldplant"><label>Plant</label><div class="static-field">Chennai Plant</div></div>' +
      '<div class="field" data-hl="fldagencyname"><label>Agency name</label><input value="Pollution Control Board" readonly /></div>' +
      '<div class="field" data-hl="flddesc"><label>Description (optional)</label><textarea rows="2" readonly>Consent, emissions and effluent filings</textarea></div></div>' +
      '<div class="modal-foot"><button class="btn btn-ghost" disabled>Cancel</button><button class="btn btn-primary" disabled>Create agency</button></div></div>';
  }

  function bodyCreateCompliance() {
    return '<div class="modal" style="margin:20px auto 0" data-hl="cascade"><div class="modal-head"><div class="icon-chip">' + icon("shield", 18) + '</div><div class="titles"><h3>Create compliance</h3></div></div>' +
      '<div class="modal-body"><div class="section-label">Location</div>' +
      '<div class="field-row" data-hl="fldlocation"><div><label>Plant</label><div class="static-field">Chennai Plant</div></div><div><label>Agency</label><div class="static-field">Labour Department</div></div></div>' +
      '<div class="field" data-hl="fldcompname"><label>Compliance name</label><input value="PF Monthly Filing" readonly /></div>' +
      '<div class="field" data-hl="fldcategory"><label>Category</label><div class="static-field">Compliance</div></div>' +
      '<div class="section-label">Assignment</div>' +
      '<div class="field-row" data-hl="fldassignment"><div><label>Department</label><div class="static-field">Safety</div></div><div><label>Owner</label><div class="static-field">Vikram Shah</div></div></div>' +
      '<div class="field" data-hl="fldreviewer"><label>Reviewer</label><div class="static-field">Rahul Gupta</div></div>' +
      '<div class="section-label">Schedule</div>' +
      '<div class="field-row" data-hl="fldschedule"><div><label>Start date</label><div class="static-field">01-08-2026</div></div><div><label>Frequency</label><div class="static-field">Every 1 month</div></div></div>' +
      '<div class="field" data-hl="flddue"><label>Next due date</label><div class="due-preview">24-08-2026</div></div></div>' +
      '<div class="modal-foot"><button class="btn btn-ghost" disabled>Cancel</button><button class="btn btn-primary" disabled>Create compliance</button></div></div>';
  }
  ...
  function sopSlides() {
    ...
    if (role === "owner") {
      ...
      slides = slides.concat([
        { mock: function () { return frame(realNavbar(), bodyHome()); }, hl: "create", title: "Create agencies & compliances", desc: "As master, the + Create button opens a menu to add a new agency or a new compliance." },

        { mock: function () { return frame(realNavbar(), bodyCreateAgency()); }, hl: "agencyform", title: "Create an agency — overview", desc: "An agency is the regulator or department a plant files with (e.g. Pollution Control Board). Every compliance sits under one. Three fields — explained next." },
        { mock: function () { return frame(realNavbar(), bodyCreateAgency()); }, hl: "fldplant", title: "Field: Plant", desc: "Choose which plant this agency belongs to. Agencies are plant-specific — Chennai and Pune each keep their own separate list." },
        { mock: function () { return frame(realNavbar(), bodyCreateAgency()); }, hl: "fldagencyname", title: "Field: Agency name", desc: "Type the regulator/department's name, e.g. 'Pollution Control Board' or 'Labour Department'. Keep it short — it shows on every compliance filed under it." },
        { mock: function () { return frame(realNavbar(), bodyCreateAgency()); }, hl: "flddesc", title: "Field: Description (optional)", desc: "A one-line note on what this agency covers, e.g. 'Consent, emissions and effluent filings'. Helps owners/reviewers see the scope at a glance — leave blank if not needed." },

        { mock: function () { return frame(realNavbar(), bodyCreateCompliance()); }, hl: "cascade", title: "Create a compliance — overview", desc: "A compliance is one specific filing obligation (e.g. PF Monthly Filing) living under an agency. A few fields to set — explained next." },
        { mock: function () { return frame(realNavbar(), bodyCreateCompliance()); }, hl: "fldlocation", title: "Field: Plant & Agency", desc: "Pick the Plant first — the Agency dropdown then loads only that plant's agencies. Every compliance must sit under one agency." },
        { mock: function () { return frame(realNavbar(), bodyCreateCompliance()); }, hl: "fldcompname", title: "Field: Compliance name", desc: "The exact filing/obligation name, e.g. 'PF Monthly Filing' or 'Consent to Operate'. This is what the owner sees in their queue, so make it specific." },
        { mock: function () { return frame(realNavbar(), bodyCreateCompliance()); }, hl: "fldcategory", title: "Field: Category", desc: "Pick from the fixed list — Return, Inspection, Renewal, Reporting, NOC, Approval, License, Tax, and more. Picking 'Return' removes 'As and When' from Frequency below, since returns need a fixed cycle." },
        { mock: function () { return frame(realNavbar(), bodyCreateCompliance()); }, hl: "fldassignment", title: "Field: Department & Owner", desc: "Department decides who's eligible below — the Owner dropdown is auto-filled live from people in that plant + department. Owner is who will actually file/fulfil this compliance." },
        { mock: function () { return frame(realNavbar(), bodyCreateCompliance()); }, hl: "fldreviewer", title: "Field: Reviewer (optional)", desc: "Also picked from the same plant + department. The reviewer only gets notified and has read-only access — leave it as 'None' if this compliance doesn't need one." },
        { mock: function () { return frame(realNavbar(), bodyCreateCompliance()); }, hl: "fldschedule", title: "Field: Start date & Frequency", desc: "Start date is when the cycle begins. Frequency is a number + unit — e.g. every 1 month, every 3 years — or 'As and When' for filings with no fixed cycle (not available when Category is Return)." },
        { mock: function () { return frame(realNavbar(), bodyCreateCompliance()); }, hl: "flddue", title: "Field: Next due date", desc: "Auto-calculated as Start date + Frequency — nothing to type, just check it looks right, then save." },
